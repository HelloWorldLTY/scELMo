...
class GEARS_Model(torch.nn.Module):
    ...
    def __init__(self, args):
        ...
        self.gene_emb_input = args['gene_emb']
        ...
        if self.gene_emb_input is None:        
            self.gene_emb = nn.Embedding(self.num_genes, hidden_size, max_norm=True)
        else:
            self.gene_emb = nn.Linear(self.gene_emb_input.shape[1], hidden_size)
        # self.gene_emb = nn.Linear(self.gene_emb_input.shape[1], hidden_size)
        # self.gene_emb = MLP([self.gene_emb_input.shape[1], hidden_size, hidden_size], last_layer_act='ReLU')
        self.gene_emb_part2 = nn.Embedding(self.num_genes, hidden_size, max_norm=True)
    ...
    def forward(self, data):
        ...
        if self.no_perturb:
            ...
            if self.gene_emb_input is None: 
                ...
                emb = self.gene_emb(torch.FloatTensor(self.gene_emb_input).to(self.args['device'])).repeat(num_graphs,1) 
                emb = emb + self.gene_emb_part2(torch.LongTensor(list(range(self.num_genes))).repeat(num_graphs, ).to(self.args['device']))   
            # print(emb.shape)     
            emb = self.bn_emb(emb)
